...
import "../utils/InitializableAbstractStrategy.sol";
import "../interfaces/external/IAave.sol";
import "../interfaces/external/IAaveStakeToken.sol";
import "../interfaces/external/IAaveIncentivesController.sol";

contract AaveStrategy is InitializableAbstractStrategy {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint16 constant public REFERRAL_CODE = 0;
    ...
    function _deposit(address _asset, uint256 _amount) internal {
        ...
        _getLendingPool().deposit(_asset, _amount, address(this), REFERRAL_CODE);
    }
    ...
    function withdrawAll() external override nonReentrant {
        for (uint256 i = 0; i < assetsMapped.length; i++) {
            ...
            uint256 balance = IERC20Upgradeable(aToken).balanceOf(address(this));
            if (balance > 0) {
                uint256 actual = _getLendingPool().withdraw(
                    address(asset),
                    balance,
        ...
        returns (uint256 balance)
    {
        ...
        balance = IERC20Upgradeable(aToken).balanceOf(address(this));
    }
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < assetsMapped.length; i++) {
            ...
            // Safe approval
            IERC20Upgradeable(asset).safeApprove(lendingPool, 0);
            IERC20Upgradeable(asset).safeApprove(lendingPool, type(uint256).max);
        }
    }
    ...
    {
        address lendingPool = address(_getLendingPool());
        IERC20Upgradeable(_asset).safeApprove(lendingPool, 0);
        IERC20Upgradeable(_asset).safeApprove(lendingPool, type(uint256).max);
    }
        ...
        nonReentrant
    {
        ...
        if (block.timestamp > windowStart && block.timestamp <= windowEnd) {
            ...
            uint256 aaveBalance = IERC20Upgradeable(rewardTokenAddresses[0]).balanceOf(
                address(this)
            );
            if (aaveBalance > 0) {
                IERC20Upgradeable(rewardTokenAddresses[0]).safeTransfer(
                    harvesterAddress,
                    aaveBalance
                );
            }
